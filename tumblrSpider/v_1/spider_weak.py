# -*- coding: utf-8 -*-
"""
Created on Tue Jun 19 20:25:42 2018
...
class tbrblog(object):
    ...
    def get_blog_name(self, url):
        name = re.findall(r'https://([^\.]+)\.tumblr\.com', url)
        if name:
            return name[0]
        else:
            print('invalid url!')
    ...
    def get_response(self, url, recu_num=0):
        try:
            ...
            if recu_num >= 3:
                ...
                print('connect url: {} failef, retry...'.format(url))
    ...
    def get_post_content(self):
        ...
        while True:
            ...
            self.Data += data
            print('crawl {} posts'.format(now))
            if now >= self.max_num or now>= self.total:
                break
            else:
...
if __name__=="__main__":
    build_proxies()
    url = input('请输入一个汤不热博客链接（形如：https://zimindoudong.tumblr.com）\n>')
    if not url:
        url = 'https://zimindoudong.tumblr.com'
    tbr = tbrblog(url)
